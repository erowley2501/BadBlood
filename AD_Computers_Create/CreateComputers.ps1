...
<#
    ...
    #=======================================================================
        ...
        if($ServerApplication -eq 0){

            $computernameprefix3 = "APPS"

        }elseif($ServerApplication -eq 1){

            $computernameprefix3 = "WEBS"

        }elseif($ServerApplication -eq 2){

            $computernameprefix3 = "DBAS"

        }elseif($ServerApplication -eq 3){

            $computernameprefix3 = "SECS"

        }elseif($ServerApplication -eq 4){

            $computernameprefix3 = "CTRX"
    ...
    if ($PSBoundParameters.ContainsKey('OUlocation') -eq $true){
        ...
        if($comps.count -eq 0){
            ...
            $compname = $computernameprefixfull + [convert]::ToInt32('10000')
        
        }else{

            try{
                $compname = $computernameprefixfull + ([convert]::ToInt32((($comps[($comps.count -1)].name).Substring(($computernameprefixfull.Length),((($comps[($comps.count -1)].name).length)-($computernameprefixfull.Length)))),10) + 1)
            }catch{
                $compname = $computernameprefixfull + [convert]::ToInt32('10000')
            }
        }
    ...
    if ($WorkstationOrServer -eq 0){
        ...
        if($WorkstationType -eq 0){ #desktop workflow
            ...
            #try{
            #    Get-ADOrganizationalUnit $oulocation|Out-Null
            #}catch{
            #    $OUlocation = 'OU=Admin,' + (Get-ADDomain).distinguishedname
            #}

            # Added by erowley2501
            $ouLocation = $OUsAll.distinguishedname | Where-Object -Filter {$_ -Like '*Desktops*' -And $_ -Like "*$computernameprefix1*"} | Get-Random
            ...
            # Added by erowley2501
            $ouLocation = $OUsAll.distinguishedname | Where-Object -Filter {$_ -Like '*Laptops*' -And $_ -Like "*$computernameprefix1*"} | Get-Random
            ...
            # Commented-out by erowley2501
            #$ouLocation = 'OU=VMs,OU=Technology,' + $dnstring
            #
            #try{
            #    Get-ADOrganizationalUnit $oulocation
            #}catch{
            ...
            # Added by erowley2501
            $ouLocation = $OUsAll.distinguishedname | Where-Object -Filter {$_ -Like '*VDIs*' -And $_ -Like "*$computernameprefix1*"} | Get-Random
    ...
    if($comps.name.count -eq 0){
        ...
        do{
            $compname = $computernameprefixfull + ([convert]::ToInt32('10000')+($i))
        ...
        do{
            ...
            try{
				$compname = $computernameprefixfull + ([convert]::ToInt32((($comps[($comps.count -1)].name).Substring(($computernameprefixfull.Length),((($comps[($comps.count -1)].name).length)-($computernameprefixfull.Length)))),10) + $i)
			}catch{
				$compname = $computernameprefixfull + ([convert]::ToInt32('10000') + ($i))
			}
    ...
    try{
		...
		New-ADComputer -server $setdc -Name $CompName -DisplayName $CompName -Enabled $true -ManagedBy $manager -SAMAccountName $sam -Description $Description
	}
